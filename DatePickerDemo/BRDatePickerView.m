...
- (void)setDateLabelText:(UILabel *)label component:(NSInteger)component row:(NSInteger)row {
    if (self.showType == BRDatePickerModeYMDHM) {
        ...
        if (component == 0) {
            label.text = [NSString stringWithFormat:@"%.2ld时", [self.hourArr[row] integerValue]];
        } else if (component == 1) {
            label.text = [NSString stringWithFormat:@"%.2ld分", [self.minuteArr[row] integerValue]];
        }
    }
//    CGFloat fontRatio = 0.66;//基线偏移比率
//    long number = 4.0;// 调整字间距
//    NSMutableAttributedString *attributedStr = [[NSMutableAttributedString alloc] initWithString:label.text];
//    [attributedStr addAttribute:NSForegroundColorAttributeName value:[UIColor orangeColor] range:NSMakeRange(label.text.length - 1, 1)];
//    [attributedStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:10.0f * kScaleFit] range:NSMakeRange(label.text.length - 1, 1)];
//    [attributedStr addAttribute:NSBaselineOffsetAttributeName value:@(fontRatio * (20.0f * kScaleFit - 10.0f * kScaleFit)) range:NSMakeRange(label.text.length - 1, 1)];
//    [attributedStr addAttribute:NSBaselineOffsetAttributeName value:@(fontRatio * (20.0f * kScaleFit - 10.0f * kScaleFit)) range:NSMakeRange(label.text.length - 1, 1)];
//    CFNumberRef num = CFNumberCreate(kCFAllocatorDefault,kCFNumberSInt8Type,&number);
//    [attributedStr addAttribute:NSKernAttributeName value:(__bridge id)num range:NSMakeRange(label.text.length - 2, 2)];
