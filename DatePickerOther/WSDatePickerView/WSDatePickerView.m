...
@interface WSDatePickerView ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate> {
    ...
    NSInteger preRow;
    NSInteger sencion0row;
    NSInteger sencion2row;
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i=0; i<nameArr.count; i++) {
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(labelX, 216/2-15/2.0, 15, 15)];
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (!customLabel) {
        ...
        if (!_selectPickerColor) {
            _selectPickerColor = [UIColor blackColor];
        }
    }
    NSString *title;
    ...
    switch (self.datePickerStyle) {
        ...
        case DateStyleShowHourMinute:
            if (component==0) {
                title = _hourArray[row];
                if (row == sencion0row) {
                    customLabel.textColor = _selectPickerColor;
                }
            }
            if (component==1) {
                title = _minuteArray[row];
...
-(UIPickerView *)datePicker {
    if (!_datePicker) {
        _datePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 216)];
